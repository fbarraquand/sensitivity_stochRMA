...
%%% Very interesting patterns there but too far away from det model
sigmaE = 0.25
%sigmaE = 0.1
%sigmaE=0.05
%------------------- Numerical integration stuff ----------------% 
intervals = 500; %50 -> dt=0.02
dt=1.0/intervals;  %-> NB decrease the timestep below theirs - they use RK4
T_max=4000; %50.0;
%timevec=linspace(0,T_max,intervals*T_max);
...
K=0.1:0.1:15
sampling_freq = 0.005
fraction_followed = 0.1 % fraction of the time series followed
length_stored=intervals*T_max*fraction_followed*sampling_freq;
...
StochCompPrey_I = sigmaE*n_I(ki)*sqrt(dt)*randn; % Standard deviation of the Wiener process increment is dt
StochCompPred_I = sigmaE*p_I(ki)*sqrt(dt)*randn;
StochCompPrey_T = sigmaE*n_T(ki)*sqrt(dt)*randn; % Standard deviation of the Wiener process increment is dt
...
%% store the end of it
nmatrix(kk,:)=n((end-intervals*T_max*fraction_followed+1):(1/sampling_freq):end);
pmatrix(kk,:)=p((end-intervals*T_max*fraction_followed+1):(1/sampling_freq):end);
n2matrix(kk,:)=n2((end-intervals*T_max*fraction_followed+1):(1/sampling_freq):end);
p2matrix(kk,:)=p2((end-intervals*T_max*fraction_followed+1):(1/sampling_freq):end);

nmatrix_I(kk,:)=n_I((end-intervals*T_max*fraction_followed+1):(1/sampling_freq):end);
pmatrix_I(kk,:)=p_I((end-intervals*T_max*fraction_followed+1):(1/sampling_freq):end);
n2matrix_I(kk,:)=n2_I((end-intervals*T_max*fraction_followed+1):(1/sampling_freq):end);
p2matrix_I(kk,:)=p2_I((end-intervals*T_max*fraction_followed+1):(1/sampling_freq):end);

nmatrix_T(kk,:)=n_T((end-intervals*T_max*fraction_followed+1):(1/sampling_freq):end);
pmatrix_T(kk,:)=p_T((end-intervals*T_max*fraction_followed+1):(1/sampling_freq):end);
n2matrix_T(kk,:)=n2_T((end-intervals*T_max*fraction_followed+1):(1/sampling_freq):end);
p2matrix_T(kk,:)=p2_T((end-intervals*T_max*fraction_followed+1):(1/sampling_freq):end);
end
...
%
print(figure(gcf),'-dpng','-r300','RMA_Kloop_largeK');
